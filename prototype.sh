...
# AWK selection: prefer gawk on Proxmox, fall back to awk
AWK_CMD="gawk"
if ! command -v "$AWK_CMD" >/dev/null 2>&1; then
    AWK_CMD="awk"
fi

# Full logging functions (defined early so traps/cleanup produce useful logs)
DEBUG=false
log_entry() {
    local level="$1"; shift
    local now; now="$(date --iso-8601=seconds 2>/dev/null || date '+%Y-%m-%dT%H:%M:%S')"
...
check_proxmox_environment() {
    ...
    mem_used_percent=$(free | grep Mem | $AWK_CMD '{print int($3/$2 * 100)}')
    ...
    if [ -f "/proc/pressure/memory" ]; then
        ...
        if [[ "$mp_val" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
            memory_pressure=$(printf "%d" "$($AWK_CMD -v v="$mp_val" 'BEGIN { printf "%f", v*100 }')" 2>/dev/null || printf "%d" 0)
        else
            memory_pressure=0
        fi
    ...
    if [ -d "/sys/devices/system/cpu/cpu0/cache/index3" ]; then
        ...
        # collect shared_cpu_list files and unique their contents to avoid duplicates
    mapfile -t ccx_files < <(find /sys/devices/system/cpu/cpu*/cache/index3 -name shared_cpu_list 2>/dev/null | while read -r f; do cat "$f"; echo ":::$f"; done | $AWK_CMD -F ':::' '{ if (!seen[$1]++) print $2 }' | sort -V)
    ...
    if command -v arcstat >/dev/null 2>&1; then
        local arc_size
    arc_size=$(arcstat -s 1 1 | tail -n 1 | $AWK_CMD '{print $2}')
        local total_mem
    total_mem=$(free -b | grep Mem | $AWK_CMD '{print $2}')
        local arc_percent
        arc_percent=0
    ...
    io_wait=$(top -bn1 | $AWK_CMD -F"," '/Cpu\(s\)/ { for(i=1;i<=NF;i++) if ($i ~ /id/) { split($i,a," "); print int(a[1]) ; exit } }' 2>/dev/null || true)
    ...
    if [ -f "/proc/pressure/cpu" ]; then
        ...
        if [[ "$cp_val" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
            cpu_pressure=$(printf "%d" "$($AWK_CMD -v v="$cp_val" 'BEGIN { printf "%f", v*100 }')" 2>/dev/null || printf "%d" 0)
        else
            cpu_pressure=0
        fi
    ...
    if [ "$POOL_HEALTH_CHECK" = true ]; then
        ...
        while IFS= read -r pool; do
            local health
            health=$(zpool status "$pool" | grep -E "state:" | $AWK_CMD '{print $2}')
...
# Colors already defined at top of script
...
Options:
  ...
  --check-resources Display current resource usage and state
    --system-check   Run optional system/Proxmox checks (disabled by default)
...
# Note: initial heavy environment validation (command checks, Proxmox checks,
# and timeout wrapper) is performed after argument parsing so lightweight
# invocations like --help or --check-resources return quickly.

# ---------- Process flags that need handling before anything else ----------
# Handle cases where no args or help flags are given first
case "${1:-}" in 
    ""|"-h"|"--help")
        show_help
        exit 0
...
# ---------- Property validation ----------
# For other flags, we need property=value as the first argument
if [[ "$1" == -* ]] || [[ "$1" != *=* ]]; then
...
# First argument should be the pattern (already shifted past property)
if [ $# -eq 0 ]; then
    echo -e "${C_BRED}Error:${C_RESET} Dataset pattern is required."
    echo "Usage: $PROG_NAME property=value pattern [options]"
    echo "Example: $PROG_NAME compression=lz4 pool/dataset/*"
    exit 4
fi
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --force)
            ...
            ;;
        *)
            echo -e "${C_BRED}Invalid flag or extra argument:${C_RESET} $1"
...
# Only require pattern if we're not running a flag-only command and have a property
if [ -n "$prop" ] && [ -z "$pattern" ]; then
    echo -e "${C_BRED}Error:${C_RESET} Dataset pattern is required when setting properties."
    echo "Usage: $PROG_NAME property=value pattern [options]"
    echo "Example: $PROG_NAME compression=lz4 pool/dataset/*"
    exit 5
fi

# If we have a pattern but no property, that's an error too (unless it was a flag-only command)
if [ -z "$prop" ] && [ -n "$pattern" ]; then
    echo -e "${C_BRED}Error:${C_RESET} Property=value required before pattern."
    echo "Usage: $PROG_NAME property=value pattern [options]"
    echo "Example: $PROG_NAME compression=lz4 pool/dataset/*"
    exit 6
fi
...
# ---------- Initial validation ----------
debug "Starting validation with property='$prop' pattern='$pattern' mode=$mode"

# Only run heavy checks if we have required args
if [ -n "$prop" ] && [ -n "$pattern" ]; then
    check_required_commands
    validate_environment
    ...
    if [ "$SYSTEM_CHECK" = true ]; then
        ...
        debug "System checks disabled (use --system-check to enable)"
    fi
fi

# Set operation timeout if we have required args and perl is available
if [ -n "$prop" ] && [ -n "$pattern" ] && command -v perl >/dev/null 2>&1; then
    TIMEOUT=${TIMEOUT:-$DEFAULT_TIMEOUT}
    export TIMEOUT
    # Only set alarm if we're not already under a timeout
    if [ -z "${ALARM_SET:-}" ]; then
        export ALARM_SET=1
        exec perl -e 'alarm $ENV{TIMEOUT}; exec @ARGV' "$0" "$@"
    fi
fi
...
# ---------- Gather targets ----------
echo "Searching for datasets matching: $pattern (mode: $mode)"

# Always try to gather targets unless it was a flag-only command
debug "Getting list of ZFS datasets..."

# Use -0 with zfs list to handle datasets with spaces
debug "Running: zfs list -H -o name"
if ! all_datasets_str=$(zfs list -H -o name 2>/dev/null); then
    echo -e "${C_BRED}Error:${C_RESET} Failed to list ZFS datasets"
    exit 6
fi

# Convert to array
mapfile -t all_datasets <<< "$all_datasets_str"
if [ ${#all_datasets[@]} -eq 0 ]; then
    echo -e "${C_BRED}Error:${C_RESET} No ZFS datasets found"
    exit 6
fi
debug "Found ${#all_datasets[@]} total datasets"

matched=()
debug "Starting pattern matching with mode=$mode pattern='$pattern_clean'"
echo "Checking for matches..."

# Remove quotes from pattern for matching
...
for ds in "${all_datasets[@]}"; do
    case "$mode" in
        glob)  
            # Use fnmatch for proper glob handling
            if [[ "$ds" == $pattern_clean ]]; then
                debug "Matched: $ds"
                matched+=("$ds")
            else
                debug "No match: $ds != $pattern_clean"
            fi
            ;;
        grep)  [[ "$ds" == *"$pattern_clean"* ]] && matched+=("$ds") ;;
        regex) [[ "$ds" =~ $pattern_clean ]] && matched+=("$ds") ;;
    esac
done
echo "Found ${#matched[@]} matching datasets"

if [ ${#matched[@]} -eq 0 ]; then
    echo -e "${C_BRED}Warning:${C_RESET} No datasets matched pattern: $pattern"
    exit 7
fi

echo -e "\n${C_BYELLOW}Matched Datasets:${C_RESET}"
printf '%s\n' "${matched[@]}" | sort
echo

# Show preview of what we'll do
print_preview "${C_BYELLOW}Will apply${C_RESET} (dry-run)"

# Exit in dry-run mode
if [ "$APPLY" = false ] || [ "$FORCE_DRY" = true ]; then
    echo -e "Dry-run complete. Use ${C_BYELLOW}--im-sure${C_RESET} to apply changes."
    exit 0
fi

# ---------- Split and validate property ----------
if [ -n "$prop" ]; then
    echo "Validating property: $prop"
    param="${prop%%=*}"
    val="${prop#*=}"
    ...
    # Check if property exists using zfs get
    debug "Checking if property '$param' exists..."
    
    # Try to get a dataset to test against
    local test_ds
    test_ds=$(zfs list -H -o name 2>/dev/null | head -n1)
    if [ -z "$test_ds" ]; then
        echo -e "${C_BRED}Error:${C_RESET} No ZFS datasets found to validate property against."
        exit 17
    fi
    
    # Test the property
    if ! zfs get -H "$param" "$test_ds" >/dev/null 2>&1; then
        echo -e "${C_BRED}Error:${C_RESET} Invalid or non-existent ZFS property: $param"
        exit 17
    fi
    debug "Property validation successful"
fi
...
print_preview() {
    local header="$1"
    echo
    echo -e "$header ${C_BMAGENTA}${param}${C_RESET}=${C_BGREEN}${val}${C_RESET}"
    echo -e "Pattern: ${C_BCYAN}$pattern${C_RESET} (${mode} matching)"
    echo -e "Datasets to modify: ${C_BYELLOW}${#matched[@]}${C_RESET}"
    echo

    # One dataset per line for clarity
    for ds in "${matched[@]}"; do
        echo -e "  ${C_BCYAN}$ds${C_RESET}"
    done
    echo
